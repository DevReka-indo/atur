                                            ...
                                            <tr class="transition hover:bg-gray-50">
                                                <td class="px-6 py-4">
                                                    <div class="flex items-center gap-3">
                                                        ...
                                                        <div class="min-w-0 flex-1">
                                                            <span
                                                                class="block text-sm font-semibold {{ $task->status === 'completed' ? 'line-through text-gray-400' : 'text-gray-900' }}">
                                                                {{ $task->name }}
                                                            </span>
                                                        </div>
                                                    </div>
                                                ...
                                                <td class="px-6 py-4">
                                                    @php
                                                        ...
                                                        // Viewer tidak bisa ubah status task
                                                        $canChangeTaskStatus =
                                                            $canContribute &&
                                                            (Auth::user()->role ?? 'member') !== 'viewer';
                                                    @endphp
                                                    ...
                                                    @if ($canChangeTaskStatus)
                                                        <button id="status-btn-{{ $task->id }}"
                                                            ...
                                                            class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold cursor-pointer hover:opacity-80 transition-opacity w-32 justify-between {{ $taskStatusColors[$task->status] ?? 'bg-gray-100 text-gray-700' }}">
                                                    ...
                                                    @else
                                                        <span
                                                            class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold cursor-default w-32 justify-center {{ $taskStatusColors[$task->status] ?? 'bg-gray-100 text-gray-700' }}"
                                                            title="Viewers cannot change task status">
                                                            {{ ucfirst(str_replace('_', ' ', $task->status)) }}
                                                        </span>
                                                    @endif
                                                </td>
                                                <td class="px-6 py-4 text-gray-600">
                                                    {{ $task->start_date?->format('d M Y') ?? '-' }}</td>
                                                <td class="px-6 py-4">
                                                    <span
                                                ...
                                                <td class="px-6 py-4">
                                                    <div class="w-full">
                                                        <div class="flex items-center justify-between mb-1">
                                                            @php
                                                                $taskProgressVal = min($progressPercentage, 100);
                                                                $taskHue = ($taskProgressVal / 100) * 120;
                                                                $taskColorStart = "hsl($taskHue, 65%, 75%)";
                                                                $taskColorEnd =
                                                                    'hsl(' . ($taskHue + 10) . ', 70%, 70%)';
                                                                $taskTextColor =
                                                                    $taskProgressVal >= 100
                                                                        ? 'text-emerald-500'
                                                                        : 'text-gray-600';
                                                            @endphp
                                                            <span
                                                                class="text-xs font-medium {{ $taskTextColor }}">{{ $progressPercentage }}%</span>
                                                        </div>
                                                        <div class="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                                                            <div class="h-2 rounded-full transition-all duration-500"
                                                                style="width: {{ $taskProgressVal }}%;
                                                                            background: linear-gradient(90deg, {{ $taskColorStart }}, {{ $taskColorEnd }});
                                                ...
                                                <td class="px-6 py-4">
                                                    <div class="table-action-icons flex items-center justify-center gap-3">
                                            ...
                                            <tr>
                                                <td colspan="7" class="px-6 py-20 text-center">
                                                    <div class="flex flex-col items-center justify-center gap-3">
                                                        ...
                                                        <p class="text-gray-700 font-semibold text-base">No tasks found</p>
                                                        <p class="text-gray-400 text-sm">Create your first task to get
                                                            started.</p>
                                                        @if ($canContribute)
                                                        @endif
                                                    </div>
