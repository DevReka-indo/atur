...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" x-data="{ showDeleteModal: false }">
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        ...
        <div class="flex gap-3"><a href="{{ route('tasks.edit', $task) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg">Edit</a>@if($isManager)<button @click="showDeleteModal = true" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg">Delete</button>@endif</div>
    </div>
            ...
            <div class="space-y-4">
                @forelse($task->statusHistory as $history)
                    <div class="relative pl-6 border-l border-gray-200">
                        ...
                        <p class="text-sm font-medium">{{ $history->from_status ? str($history->from_status)->replace('_',' ')->title() : 'Created' }} → {{ str($history->to_status)->replace('_',' ')->title() }}</p>
